...
if [[ $1 == "sink" ]]; then
  ...
  mkdir -p out
  rm -f out/sink.zip
  zip -r out/sink.zip . -x package.json -x yarn.lock -x README.md -x deploy.sh -x "*node_modules*" -x "*.DS_Store*" -x "*.venv*" -x "*.idea*" -x ".gitignore" -x "*.git*" -x "*out*" -x test.js -x worker.js
...
if [[ $1 == "worker" ]]; then
  ...
  mkdir -p out
  rm -f out/worker.zip
  zip -r out/worker.zip . -x package.json -x yarn.lock -x README.md -x deploy.sh -x "*node_modules*" -x "*.DS_Store*" -x "*.venv*" -x "*.idea*" -x ".gitignore" -x "*.git*" -x "*out*" -x test.js -x index.js
  echo "deploying to λ function..."
  cd out
